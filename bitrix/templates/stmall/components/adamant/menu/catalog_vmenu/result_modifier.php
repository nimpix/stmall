...
foreach($arResult as $key=>$arItem)
{
	...
	$arItem["PARAMS"]["item_id"] = $arItem["LINK"];
...
foreach ($arResult['ALL_ITEMS'] as $arRes){
	//вытаскиваем из ссылки раздела символьный CODE раздела
    $arExplode = explode('/',$arRes['LINK']);
    $arLength = count($arExplode);
    $arResLink = $arExplode[$arLength-2];

   //по символьному коду получем ID раздела
    $res = CIBlockSection::GetList(array(), array('IBLOCK_ID' => 1, 'CODE' => $arResLink, 'SITE_ID' => "s1"));
    $section = $res->Fetch();


//по ID раздела получаем элементы раздела(товары)
    $res_elem = CIBlockElement::GetList(
        array(),
        array("IBLOCK_ID" => 1, "SECTION_ID" => $section["ID"], "INCLUDE_SUBSECTIONS" => "Y"),
        false,
        false,
        array("ID", "IBLOCK_ID", "LANG_DIR")
    );

    $cnt = 0;
   //Считаем количество товаров в разделе
    while($ob = $res_elem->GetNext())
    {
        $cnt++;
...
$cnt_items = 0;
//Добавляем в результирующий массив свойство для каждого раздела -- количество товаров count_of_el
foreach ($arResult['ALL_ITEMS'] as &$item){
    $item['PARAMS']['count_of_el'] = $myResult[$cnt_items];
    $cnt_items++;
}
unset($item);
