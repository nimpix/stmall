...
$currencyList = '';
    ...
    false,
    false,
    array("ID", "IBLOCK_ID", "LANG_DIR", "DETAIL_PAGE_URL", "PROPERTY_BREND", "SHOW_COUNTER")
  	); 
...
?>
<div class="bx-catalog-element bx-moll bx-<?=$arParams['TEMPLATE_THEME']?>" id="<?=$itemIds['ID']?>"
		...
		<div class="row">
			<div class="col-sm-8 col-md-9">
				<div class="view-panel">
					<div class="cnt">����� ����������: <?=$tmps["COUNTER"];?></div>
				</div>
		...
		<div class="row">
			...
			<div style="display: none;" id="222">
				<? 
					...
					if (!empty($tmps["BREND_VAL"])){
			            if (CModule::IncludeModule('highloadblock')){
			               ...
			               $hlblock = Bitrix\Highloadblock\HighloadBlockTable::getById(1)->fetch(); // id highload блока (например, бренда)
			               $entity = Bitrix\Highloadblock\HighloadBlockTable::compileEntity($hlblock);
			               $entityClass = $entity->getDataClass();
			
			               $resx = $entityClass::getList(array(
			                   'select' => array('ID','UF_NAME'),
			                   'filter' => array('UF_XML_ID' => $tmps["BREND_VAL"]) // $prop_val - это машинное значение свойства, например 'marcotozzi'
			               ));
			
			                if ($erow = $resx->fetch())     
			                   $tmps["BREND_NAME"] = $erow['UF_NAME'];  //здесь вернется читабельное значение свойства, например "Marco Tozzi"
					...
					if (isset($arResult['DISPLAY_PROPERTIES']['brend']['VALUE'])){
						$brend_path = 'f/brend-is-'.$arResult['DISPLAY_PROPERTIES']['brend']['VALUE'];
				...
				<div class="products_links">
					...
					<? if (isset($arResult['SECTION']['PATH'][1])){?>
						<li><a href="<?=$basePath.$arResult['SECTION']['PATH'][0]['SECTION_PAGE_URL']?>">Все <? print strtolower($arResult['SECTION']['PATH'][0]['NAME']); ?> </a></li>
					<? }?>
					</ul>
